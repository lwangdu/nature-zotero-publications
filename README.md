...
Zotero limits an API response to 100 items. Version 1.0.1 adds a resumable background synchronization system in `includes/class-sync.php`:
...
Visitors continue to see the last completed generation while a refresh is running. During the first synchronization, the plugin uses its transient-backed fallback, which can hold up to 2,000 items. A seven-day stale fallback helps keep the bibliography available when Zotero is temporarily unreachable.
...
Large libraries require multiple background requests. Visit the page once to ensure the source is scheduled, confirm WordPress Cron can run, and allow the synchronization to finish. The existing completed data remains visible during later refreshes.
